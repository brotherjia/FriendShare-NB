...
function IgnoreShare_Command(command)
	...
	if ((cmd == "") or (cmd == "help")) then
		...
		IgnoreShare_ChatPrint( "用法：" );
		IgnoreShare_ChatPrint(string.format(lineFormat, "<help>", "显示帮助信息。"));
		IgnoreShare_ChatPrint(string.format(lineFormat, "reset", "重置全局屏蔽列表。"));
		IgnoreShare_ChatPrint(string.format(lineFormat, "import", "导入全局屏蔽列表。"));
	end
	if (cmd == "reset" ) then
	...
	if ( cmd == "import" ) then
		IgnoreShare_Import();
		FriendShare_ChatPrint( "IgnoreShare: 全局屏蔽列表已导入。" );
	end
end
...
function IgnoreShare_Import()
	...
	for i, name in pairs( globalIgnores ) do
		...
		then
			IgnoreShare_ChatPrint( "IgnoreShare: 正在屏蔽 " .. name );
			AddIgnore( name );
		end
	end
	...
	IgnoreShare_ChatPrint( "IgnoreShare: 全局屏蔽列表已导入。" );
end
...
function IgnoreShare_UpdateGlobalIgnores(ignoresList)
	for i, name in pairs( ignoresList ) do
		if ( not IgnoreShare_GlobalIgnores[realmAndFaction][name] and
			not IgnoreShare_RemovedIgnores[realmAndFaction][name] )
		then
			IgnoreShare_ChatPrint( "IgnoreShare: 正在添加全局屏蔽 " .. name );
